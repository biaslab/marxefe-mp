...
BayesBase.default_prod_rule(::Type{<:MatrixNormal}, ::Type{<:MatrixNormal}) = BayesBase.PreserveTypeProd(Distribution)

function BayesBase.prod(::BayesBase.PreserveTypeProd{Distribution}, left::MatrixNormal, right::MatrixNormal)
    
    Dx,Dy = BayesBase.dim(left)
    Ml, Ul, Vl = RxInfer.params(left)
    Mr, Ur, Vr = RxInfer.params(right)
